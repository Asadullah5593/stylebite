...
<div class="posts-page space-y-5">
    <nav class="d-flex align-items-center gap-2 mb-3 small opacity-75">
        ...
        <i class="bi bi-chevron-right small"></i>
        <span class="fw-bold opacity-50">Post Media</span>
    </nav>
    ...
    <form method="GET" action="{{ route('admin.posts.post_media') }}" class="glass rounded-4 p-3 d-flex flex-wrap align-items-center gap-3 mb-4">
        <div class="position-relative flex-grow-1" style="min-width: 250px;">
            <i class="bi bi-search position-absolute top-50 start-0 translate-middle-y ms-3 text-muted"></i>
            <input type="text" name="q" value="{{ request('q') }}" class="form-control ps-5 border-0 bg-dark-soft rounded-3" placeholder="Search media, role, status, post...">
        </div>
        ...
        <a href="{{ route('admin.posts.post_media') }}" class="btn btn-outline-dynamic rounded-3 px-3"><i class="bi bi-arrow-clockwise me-2"></i>Reset</a>
    </form>
    ...
    <div class="glass rounded-4 overflow-hidden border-white-05">
                ...
                <tbody>
                    @forelse ($media as $item)
                        <tr class="border-white-05">
                            ...
                            <td><span class="badge {{ $item->processing_status === 'ready' ? 'bg-success-soft text-success' : 'bg-warning-soft text-warning' }} rounded-pill">{{ str($item->processing_status)->title() }}</span></td>
        ...
        <div class="p-4 bg-white-05 border-top border-white-05 d-flex align-items-center justify-content-between flex-wrap gap-3">
            <div class="text-muted small">
                Showing <span class="text-emphasis-dynamic fw-bold">{{ $media->firstItem() ?? 0 }}-{{ $media->lastItem() ?? 0 }}</span>
                of <span class="text-emphasis-dynamic fw-bold">{{ number_format($media->total()) }}</span> media records
            </div>
            {{ $media->links() }}
        </div>
    </div>
